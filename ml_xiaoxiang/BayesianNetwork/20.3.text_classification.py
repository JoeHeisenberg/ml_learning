...
def test_clf(clf):
    ...
    print
    u'5折交叉验证的训练时间为：%.3f秒/(5*%d)=%.3f秒' % ((t_end - t_start), m, t_train)
    print
    u'最优超参数为：', model.best_params_
    t_start = time()
...
if __name__ == "__main__":
    ...
    t_end = time()
    print
    u'下载/加载数据完成，耗时%.3f秒' % (t_end - t_start)
    print
    u'数据类型：', type(data_train)
    print
    u'训练集包含的文本数目：', len(data_train.data)
    print
    u'测试集包含的文本数目：', len(data_test.data)
    print
    ...
    for i in np.arange(10):
        print
        u'文本%d(属于类别 - %s)：' % (i + 1, categories[y_train[i]])
        print
        data_train.data[i]
        print
        '\n\n'
    ...
    print
    u'训练集样本个数：%d，特征个数：%d' % x_train.shape
    print
    u'停止词:\n',
